...
check_server() {
    if ! curl -s -o /dev/null --head "$JETSTREAM_URL"; then
...
# Main test function
run_tests() {
    echo "Starting JetstreamDB tests..."
    
    # Check if server is running
    check_server
    
    # Test 1: Basic storage and retrieval
    run_test "Store content" "echo -n '$TEST_CONTENT' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL'" 0 "/a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3.dat"
    
    # Get the hash of the test content
    HASH=$(echo -n "$TEST_CONTENT" | sha256sum | cut -d' ' -f1)
    
    # Test 2: Retrieve stored content
    run_test "Retrieve stored content" "curl -s '$JETSTREAM_URL/$HASH.dat'" 0 "$TEST_CONTENT"
    
    # Test 3: Try to delete content (should fail as it's read-only)
    run_test "Attempt to delete read-only content" "curl -s -X DELETE '$JETSTREAM_URL/$HASH.dat'" 0
    
    # Verify content still exists
    run_test "Verify content still exists" "[ \"\$(curl -s '$JETSTREAM_URL/$HASH.dat')\" = \"$TEST_CONTENT\" ]" 0
    ...
    run_test "Store key-value pair" "echo -n 'value1' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$KEY_HASH.dat?format=$JETSTREAM_URL*'" 0 "$JETSTREAM_URL/$KEY_HASH.dat"
    
    # Test 6: Retrieve key-value pair
    run_test "Retrieve key-value pair" "curl -s '$JETSTREAM_URL/$KEY_HASH.dat'" 0 "value1"
    
    # Test 7: Update key-value pair
    run_test "Update key-value pair" "echo -n 'value2' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$KEY_HASH.dat?format=$JETSTREAM_URL*'" 0 "$JETSTREAM_URL/$KEY_HASH.dat"
    
    # Test 8: Verify update
    run_test "Verify updated value" "[ \"\$(curl -s '$JETSTREAM_URL/$KEY_HASH.dat')\" = \"value2\" ]" 0
    
    # Test 9: Delete key-value pair
    run_test "Delete key-value pair" "curl -s -X DELETE '$JETSTREAM_URL/$KEY_HASH.dat'" 0 "/$KEY_HASH.dat"
    
    # Test 10: Verify deletion returns empty response
    run_test "Verify deletion returns empty response" "[ \"\$(curl -s '$JETSTREAM_URL/$KEY_HASH.dat')\" = \"\" ]" 0
    ...
    run_test "Store burst file 1" "echo -n 'burst_content_1' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$BURST_HASH1.dat'" 0 "/$BURST_HASH1.dat"
    run_test "Store burst file 2" "echo -n 'burst_content_2' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$BURST_HASH2.dat'" 0 "/$BURST_HASH2.dat"
    run_test "Store burst file 3" "echo -n 'burst_content_3' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$BURST_HASH3.dat'" 0 "/$BURST_HASH3.dat"
    ...
    run_test "Upload burst list file" "cat '$BURST_LIST' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$BURST_INDEX_HASH.dat'" 0 "/$BURST_INDEX_HASH.dat"
    
    # Test burst without burst=1 (should return the list of files with newlines)
    run_test "Burst without parameter returns file list" "[ \"\$(curl -s '$JETSTREAM_URL/$BURST_INDEX_HASH.dat')\" = \"$(cat "$BURST_LIST")\" ]" 0
    
    # Test burst with burst=1 (should return concatenated content)
    run_test "Burst with parameter returns concatenated content" "[ \"\$(curl -s '$JETSTREAM_URL/$BURST_INDEX_HASH.dat?burst=1')\" = \"burst_content_1burst_content_2burst_content_3\" ]" 0
    ...
    run_test "Sync: setifnot first attempt should return path" "printf '123' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$SYNC_KEY.dat?setifnot=1'" 0 "/$SYNC_KEY.dat"
    run_test "Sync: setifnot second attempt should return empty" "[ -z "$(echo 456 | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$SYNC_KEY.dat?setifnot=1')" ]" 0
    
    # Test append=1
    run_test "Sync: append to file" "printf ' line2' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$SYNC_KEY.dat?append=1'" 0 "/$SYNC_KEY.dat"
    run_test "Sync: verify append" "curl -s '$JETSTREAM_URL/$SYNC_KEY.dat'" 0 $'123 line2'
    
    # Test take=1 (should return content and delete)
    run_test "Sync: first take operation" "curl -s '$JETSTREAM_URL/$SYNC_KEY.dat?take=1'" 0 $'123 line2'
    run_test "Sync: second take should return empty" "curl -s '$JETSTREAM_URL/$SYNC_KEY.dat?take=1'" 0 ""
    ...
    run_test "Auth: store read-only block" "echo -n '$READONLY_CONTENT' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$READONLY_HASH.dat'" 0 "/$READONLY_HASH.dat"
    
    # Create a mutable pointer to the read-only block
    run_test "Auth: create mutable pointer" "echo -n '/$READONLY_HASH.dat' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$MUTABLE_POINTER.dat'" 0 "/$MUTABLE_POINTER.dat"
    
    # Verify the pointer works with burst=1
    run_test "Auth: verify pointer with burst=1" "curl -s '$JETSTREAM_URL/$MUTABLE_POINTER.dat?burst=1'" 0 'This is a read-only block.'
    
    # Try to modify the read-only block (should fail)
    run_test "Auth: attempt to modify read-only block" "[ \"$(echo 'Modified content' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL/$READONLY_HASH.dat')\" = '' ]" 0
    
    # Verify the read-only block is unchanged
    run_test "Auth: verify read-only block unchanged" "curl -s '$JETSTREAM_URL/$READONLY_HASH.dat'" 0 'This is a read-only block.'
    ...
    run_test "Channel: create write channel" "curl -s -X PUT '$JETSTREAM_URL$CHANNEL_PATH' -d '$CHANNEL_CONTENT'" 0 "$CHANNEL_PATH"
    
    # Write "123" through the channel - should redirect to target
    run_test "Channel: write 123 through channel" "echo '123' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL$CHANNEL_PATH'" 0 "$CHANNEL_PATH"
    
    # Read "123" from target file immediately after first write
    run_test "Channel: read 123 from target file" "curl -s '$JETSTREAM_URL$TARGET_PATH'" 0 "123"
    
    # Read from channel file - should always return empty string
    run_test "Channel: read from channel returns empty (after 123)" "curl -s '$JETSTREAM_URL$CHANNEL_PATH'" 0 ""
    
    # Write "456" through the channel - should redirect to target  
    run_test "Channel: write 456 through channel" "echo '456' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL$CHANNEL_PATH'" 0 "$CHANNEL_PATH"
    
    # Read "456" from target file immediately after second write
    run_test "Channel: read 456 from target file" "curl -s '$JETSTREAM_URL$TARGET_PATH'" 0 "456"
    
    # Read from channel file - should always return empty string
    run_test "Channel: read from channel returns empty (after 456)" "curl -s '$JETSTREAM_URL$CHANNEL_PATH'" 0 ""
    
    # Test channel deletion protection - should fail to delete
    run_test "Channel: deletion protection" "[ \"\$(curl -s -X DELETE '$JETSTREAM_URL$CHANNEL_PATH')\" = '' ]" 0
    
    # Verify channel still exists after attempted deletion
    run_test "Channel: verify channel exists after failed delete" "curl -s '$JETSTREAM_URL$CHANNEL_PATH'" 0 ""
    
    # Final test: verify data persistence after failed channel deletion
    run_test "Channel: read 456 from target file after failed delete" "curl -s '$JETSTREAM_URL$TARGET_PATH'" 0 "456"
    ...
    run_test "Append Channel: create append channel" "curl -s -X PUT '$JETSTREAM_URL$APPEND_CHANNEL_PATH' -d '$APPEND_CHANNEL_CONTENT'" 0 "$APPEND_CHANNEL_PATH"
    
    # Write "123" through the append channel - should redirect to target with append=1
    run_test "Append Channel: write 123 through channel" "printf '123' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL$APPEND_CHANNEL_PATH'" 0 "$APPEND_CHANNEL_PATH"
    
    # Read "123" from target file immediately after first write
    run_test "Append Channel: read 123 from target file" "curl -s '$JETSTREAM_URL$APPEND_TARGET_PATH'" 0 "123"
    
    # Read from append channel file - should always return empty string
    run_test "Append Channel: read from channel returns empty (after 123)" "curl -s '$JETSTREAM_URL$APPEND_CHANNEL_PATH'" 0 ""
    
    # Write "456" through the append channel - should append to target  
    run_test "Append Channel: write 456 through channel" "printf '456' | curl -s -X PUT --data-binary @- '$JETSTREAM_URL$APPEND_CHANNEL_PATH'" 0 "$APPEND_CHANNEL_PATH"
    
    # Read "123456" from target file after append operation
    run_test "Append Channel: read 123456 from target file" "curl -s '$JETSTREAM_URL$APPEND_TARGET_PATH'" 0 "123456"
    
    # Read from append channel file - should always return empty string
    run_test "Append Channel: read from channel returns empty (after 456)" "curl -s '$JETSTREAM_URL$APPEND_CHANNEL_PATH'" 0 ""
    
    # Test append channel deletion protection - should fail to delete
    run_test "Append Channel: deletion protection" "[ \"\$(curl -s -X DELETE '$JETSTREAM_URL$APPEND_CHANNEL_PATH')\" = '' ]" 0
    
    # Verify append channel still exists after attempted deletion
    run_test "Append Channel: verify channel exists after failed delete" "curl -s '$JETSTREAM_URL$APPEND_CHANNEL_PATH'" 0 ""
    
    # Final test: verify data persistence after failed append channel deletion
    run_test "Append Channel: read 123456 from target file after failed delete" "curl -s '$JETSTREAM_URL$APPEND_TARGET_PATH'" 0 "123456"
    ...
    run_test "Channel: write to read channel should fail" \
        "curl -s -X PUT \"$JETSTREAM_URL$CHANNEL_PATH\" -d '789'" \
        0 ""
    ...
    run_test "Channel: delete read channel should fail" \
        "curl -s -X DELETE \"$JETSTREAM_URL$CHANNEL_PATH\"" \
        0 ""
    ...
    run_test "Channel: volatile mode returns empty for read channel" \
        "curl -s -X GET \"$JETSTREAM_URL$CHANNEL_PATH?volatile=1\"" \
        0 ""
